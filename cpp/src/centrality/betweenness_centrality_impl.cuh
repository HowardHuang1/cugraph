 ...
 */
 ...
 {
   //
   // Concurrent multi-source BFS using tagged frontiers
   // Each source is tagged with its source index for tracking
   //
   // This function performs concurrent BFS from multiple sources simultaneously,
   // which is more efficient than running separate BFS for each source.
   // 
   // Key benefits:
   // 1. Better GPU utilization - multiple frontiers are expanded concurrently
   // 2. Reduced memory overhead - shared data structures across sources
   // 3. Improved performance for betweenness centrality with many sources
   //
   // Usage:
   //   auto [distances, sigmas] = multisource_bfs(handle, graph_view, edge_weight_view, sources, do_expensive_check);
   //   multisource_backward_pass(handle, graph_view, edge_weight_view, centralities, std::move(distances), std::move(sigmas), sources, with_endpoints, do_expensive_check);
   //
   constexpr vertex_t invalid_distance = std::numeric_limits<vertex_t>::max();
   constexpr size_t bucket_idx_cur{0};
   constexpr size_t bucket_idx_next{1};
 
   using source_idx_t = uint32_t;  // Source index type
   ...
   // Create tagged frontier with source indices
   vertex_frontier_t<vertex_t, source_idx_t, multi_gpu, true> vertex_frontier(handle, 2);
 
   auto vertex_partition =
     vertex_partition_device_view_t<vertex_t, multi_gpu>(graph_view.local_vertex_partition_view());
 
   // Initialize sources with their indices
   if (sources.size() > 0) {
     rmm::device_uvector<vertex_t> local_sources(sources.size(), handle.get_stream());
     rmm::device_uvector<source_idx_t> source_indices(sources.size(), handle.get_stream());
     
     // Copy sources and create indices
     raft::copy(local_sources.data(), sources.data(), sources.size(), handle.get_stream());
     thrust::sequence(handle.get_thrust_policy(), source_indices.begin(), source_indices.end(), source_idx_t{0});
     
     // Insert tagged sources into frontier
     vertex_frontier.bucket(bucket_idx_cur).insert(
       thrust::make_zip_iterator(local_sources.begin(), source_indices.begin()),
       thrust::make_zip_iterator(local_sources.end(), source_indices.end()));
     ...
     thrust::for_each(
       ...
       [d_sigma_2d = sigmas_2d.begin(), d_distance_2d = distances_2d.begin(), vertex_partition, num_vertices] __device__(
         ...
         auto source_idx = thrust::get<1>(tagged_source);
         auto offset = vertex_partition.local_vertex_partition_offset_from_vertex_nocheck(v);
         auto idx = source_idx * num_vertices + offset;
         d_distance_2d[idx] = 0;
         d_sigma_2d[idx] = 1;
       });
   }
   ...
   while (vertex_frontier.bucket(bucket_idx_cur).aggregate_size() > 0) {
     // Use extract_transform_if_v_frontier_e for truly parallel multi-source BFS
     // This function extracts raw edge data, which we'll convert back to frontier format
     
     // Allocate dataframe buffers first (following OD Shortest Distances pattern)
     auto new_frontier_tagged_vertex_buffer = 
       allocate_dataframe_buffer<thrust::tuple<vertex_t, source_idx_t>>(0, handle.get_stream());
     auto new_sigma_buffer = 
       allocate_dataframe_buffer<edge_t>(0, handle.get_stream());
     
     // Extract and transform edges from the tagged frontier
     // Since we're doing unweighted BFS, we always use dummy edge weights
     std::tie(new_frontier_tagged_vertex_buffer, new_sigma_buffer) = detail::
       extract_transform_if_v_frontier_e<false, thrust::tuple<vertex_t, source_idx_t>, edge_t>(
         handle,
         graph_view,
         vertex_frontier.bucket(bucket_idx_cur),
         ...
         [d_sigma_2d = sigmas_2d.begin(), d_distance_2d = distances_2d.begin(),
          vertex_partition, num_vertices] __device__(auto tagged_src, auto dst, auto, auto, auto edge_weight) {
           ...
           auto src_idx = source_idx * num_vertices + src_offset;
           auto src_sigma = d_sigma_2d[src_idx];
           // Return tagged destination with sigma value
           return thrust::make_tuple(thrust::make_tuple(dst, source_idx), src_sigma);
         },
         [d_distance_2d = distances_2d.begin(), vertex_partition, num_vertices, hop] __device__(
           ...
           auto dst_idx = source_idx * num_vertices + dst_offset;
           auto dst_distance = d_distance_2d[dst_idx];
           // Only process edges to unvisited vertices for this source
           return (dst_distance == std::numeric_limits<vertex_t>::max());
         });
     
     // Convert the results back to a format that update_v_frontier can use
     vertex_frontier_t<vertex_t, source_idx_t, multi_gpu, true> new_vertex_frontier(handle, 2);
     
     // Insert the tagged vertices into the new frontier directly
     if (size_dataframe_buffer(new_frontier_tagged_vertex_buffer) > 0) {
       // Convert dataframe buffer to device vector first
       rmm::device_uvector<thrust::tuple<vertex_t, source_idx_t>> frontier_vertices(
       ...
       new_vertex_frontier.bucket(bucket_idx_next).insert(
         frontier_vertices.begin(),
         frontier_vertices.end());
     }
     
     // Convert sigma buffer to device vector for update_v_frontier
     rmm::device_uvector<edge_t> new_sigmas(0, handle.get_stream());
     if (size_dataframe_buffer(new_sigma_buffer) > 0) {
       new_sigmas.resize(size_dataframe_buffer(new_sigma_buffer), handle.get_stream());
       thrust::copy(handle.get_thrust_policy(),
     ...
     resize_dataframe_buffer(new_sigma_buffer, 0, handle.get_stream());
     shrink_to_fit_dataframe_buffer(new_sigma_buffer, handle.get_stream());
     
     // Update the main tagged frontier with new vertices and update 2D arrays
     auto next_frontier_bucket_indices = std::vector<size_t>{bucket_idx_next};
     update_v_frontier(handle,
                       graph_view,
                       std::move(new_frontier_tagged_vertex_buffer),
                       std::move(new_sigma_buffer),
                       vertex_frontier,
                       raft::host_span<size_t const>(next_frontier_bucket_indices.data(),
                                                     next_frontier_bucket_indices.size()),
                       thrust::make_zip_iterator(distances_2d.begin(), sigmas_2d.begin()),
                       thrust::make_zip_iterator(distances_2d.begin(), sigmas_2d.begin()),
                       [hop, num_vertices, vertex_partition, d_distances_2d = distances_2d.begin(), d_sigmas_2d = sigmas_2d.begin()] __device__(auto tagged_v, auto old_values, auto v_sigma) {
                         auto v = thrust::get<0>(tagged_v);
                         auto source_idx = thrust::get<1>(tagged_v);
                         auto offset = vertex_partition.local_vertex_partition_offset_from_vertex_nocheck(v);
                         auto idx = source_idx * num_vertices + offset;
                         
                         // Check the current value in the 2D array for this specific source
                         auto current_distance = d_distances_2d[idx];
                         
                         // Only update if this is a new discovery by this source (distance == invalid_distance)
                         if (current_distance == std::numeric_limits<vertex_t>::max()) {
                           // Update the 2D arrays directly
                           d_distances_2d[idx] = hop + 1;
                           d_sigmas_2d[idx] = v_sigma;
                           
                           // Return bucket index and new value for update_v_frontier
                           return thrust::make_tuple(
                             cuda::std::make_optional(bucket_idx_next),
                             cuda::std::make_optional(thrust::make_tuple(hop + 1, v_sigma)));
                         } else {
                           // Already discovered by this source, don't add to frontier
                           // Return nullopt for both bucket and value
                           return thrust::make_tuple(
                             cuda::std::optional<size_t>{cuda::std::nullopt},
                             cuda::std::optional<thrust::tuple<vertex_t, edge_t>>{cuda::std::nullopt});
                         }
                       });
 ...
 {
   //
   // Backward pass for multi-source betweenness centrality
   // This processes each source separately and accumulates results
   //
   constexpr vertex_t invalid_distance = std::numeric_limits<vertex_t>::max();
   auto num_vertices = graph_view.local_vertex_partition_range_size();
   auto num_sources = sources.size();
   ...
   for (size_t source_idx = 0; source_idx < num_sources; ++source_idx) {
     // Extract single-source distances and sigmas
     ...
     // Copy data for this source
     thrust::copy(
       handle.get_thrust_policy(),
       distances.begin() + source_idx * num_vertices,
       distances.begin() + (source_idx + 1) * num_vertices,
       single_distances.begin());
     thrust::copy(
       handle.get_thrust_policy(),
       sigmas.begin() + source_idx * num_vertices,
       sigmas.begin() + (source_idx + 1) * num_vertices,
       single_sigmas.begin());
     ...
     rmm::device_uvector<weight_t> deltas(single_sigmas.size(), handle.get_stream());
     ...
     // Create separate property objects for distances, sigmas, and deltas
     ...
     update_edge_src_property(
       handle,
       graph_view,
       thrust::make_zip_iterator(single_distances.begin(), single_sigmas.begin(), deltas.begin()),
     ...
     rmm::device_uvector<vertex_t> vertices_sorted(single_distances.size(), handle.get_stream());
     {
       // Create distance_keys for sorting
       rmm::device_uvector<vertex_t> distance_keys(single_distances.size(), handle.get_stream());
       
       // Copy distances for sorting (preserve original)
       raft::copy(distance_keys.data(), single_distances.data(), single_distances.size(), handle.get_stream());
   ...
   bool const do_expensive_check)
 {
   ...
   //
   // Use truly parallel multi-source BFS for better performance
   // This processes all sources simultaneously instead of sequentially
   //
   ...
   if constexpr (multi_gpu) {
     // For multi-GPU, we need to handle source distribution
     auto local_source_count = source_offsets[my_rank + 1] - source_offsets[my_rank];
     if (local_source_count > 0) {
       thrust::copy(handle.get_thrust_policy(),
                    vertices_begin,
                    vertices_begin + local_source_count,
                    sources_buffer.begin());
     }
   } else {
     // Single GPU - copy all sources
     thrust::copy(handle.get_thrust_policy(),
                  vertices_begin,
                  vertices_end,
                  sources_buffer.begin());
   ...
   // Perform truly parallel multi-source BFS
   auto [distances_2d, sigmas_2d] = multisource_bfs(
     handle,
     graph_view,
     edge_weight_view,
   ...
   // Use existing single-source backward pass for testing
